...
class ArticleSummaryTests(unittest.TestCase):
    def test_uses_gemini_summary_when_it_differs_from_preview(self):
        client = FakeGeminiClient(
            ["Patreon sépare désormais le calcul et l'envoi des notifications pour absorber les pics."]
        )
    ...
    def test_retries_once_when_first_summary_duplicates_preview(self):
        ...
        rewritten = "La plateforme remplace une tâche monolithique par un traitement en deux étapes."
    ...
    def test_falls_back_to_existing_summarizer_on_api_error(self):
        ...
        with patch("utils.summarizer.generate_summary", return_value="Résumé de secours") as fallback:
            summary = generate_article_summary(
                title="Title",
    ...
    def test_falls_back_without_calling_unconfigured_client(self):
        ...
        with patch("utils.summarizer.generate_summary", return_value="Résumé de secours"):
            summary = generate_article_summary(
                title="Title",
